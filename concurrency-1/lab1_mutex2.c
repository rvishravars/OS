...
static int __init my_init(void)
{
	...
	/* COMPLETE ME: try to acquire without sleeping; fail to load if busy */
	if (!mutex_trylock(&my_mutex)) {
		printk(KERN_INFO "%s: mutex is busy; refusing to load\n", KBUILD_MODNAME);
		return -EBUSY;
	}
	locked_by_me = true;
	printk(KERN_INFO "%s: mutex acquired\n", KBUILD_MODNAME);
...
{
	/* COMPLETE ME: release if we held it */
	if (locked_by_me) {
